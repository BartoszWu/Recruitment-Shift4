...
describe('moneyInput:utils', () => {
    ...
    describe('formatNumericValue', () => {
        ...
        it('should remove text ending on double decimal point', () => {
            expect(formatNumericValue('12..', '12.')).toBe('12.')
        })

        it('should remove double decimal point', () => {
            expect(formatNumericValue('1.2.00', '12.00')).toBe('12.00')
        })

        it('should not allow decimal point before thousands separator', () => {
            expect(formatNumericValue('12.3,4', '12.34')).toBe('12.34')
        })

        it('should handle values ending with zero in decimal part', () => {
            expect(formatNumericValue('1234.50', '1234.5')).toBe('1,234.50')
        })

        it('should return empty string for invalid input', () => {
            expect(formatNumericValue('a', '')).toBe('0')
        })

        it('should handle leading and trailing non-numeric characters', () => {
            expect(formatNumericValue('1234x', '1234')).toBe('1,234')
        })

        it('should handle numeric strings with spaces and special characters', () => {
            expect(formatNumericValue(' 1,234 ', '1,234')).toBe('1,234')
            expect(formatNumericValue('1,234$%^', '1,234$')).toBe('1,234')
        })

        it('should correctly format large numbers with commas', () => {
            expect(formatNumericValue('1000000', '100000')).toBe('1,000,000')
            expect(formatNumericValue('1000000.99', '1,000,000.9')).toBe(
                '1,000,000.99'
            )
        })

        it('should correctly format numbers with 00 in decimal places', () => {
            expect(formatNumericValue('1234.0', '1234.')).toBe('1,234.0')
